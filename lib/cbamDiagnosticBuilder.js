 ...
 * El pre-relleno real se inyecta via localStorage en el cliente; aquí solo
 * señalamos el paquete sugerido y el origen.
 */
export function buildCtaUrl(recommendedPackage) {
  const typeParam = {
    basico: 'basico',
    completo: 'completo',
    monitorizacion: 'basico', // el wizard Advisory no tiene "monitorización" como tipo; le damos básico + señal from
  }[recommendedPackage] || 'basico'
  return `/cbam/asesoria/solicitud?tipo=${typeParam}&from=diagnostic`
}
