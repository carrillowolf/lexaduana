...
// Fecha de publicación oficial del precio por la Comisión Europea (formato ISO).
// NO es el trimestre de aplicación: el precio publicado en abril es la media
// del trimestre anterior y aplica a importaciones del trimestre anterior.
export const CBAM_CERTIFICATE_PRICE_PUBLICATION_DATE = '2026-04-08'
// Trimestre al que aplica el precio (formato YYYY-QN). Es lo que importa al
// cliente: el trimestre de sus importaciones cubierto por este precio.
export const CBAM_CERTIFICATE_PRICE_APPLICATION_QUARTER = '2026-Q1'
// Alias legacy mantenido por compatibilidad con snapshots antiguos. Apunta al
// trimestre de aplicación (que es lo que el PDF venía mostrando en la práctica).
export const CBAM_CERTIFICATE_PRICE_DATE = CBAM_CERTIFICATE_PRICE_APPLICATION_QUARTER
// Referencia regulatoria del precio. NO confundir con "lookup source"
// (que indica si el valor en runtime vino de BD o de la constante de
// respaldo, y se llama certificatePriceLookupSource en el motor).
export const CBAM_CERTIFICATE_PRICE_REGULATORY_REF = 'Reg. Ejecución (UE) 2025/2548'

export function getCbamCertificatePrice() {
  return {
    price: CBAM_CERTIFICATE_PRICE_FALLBACK,
    // Legacy: el PDF venía mostrando esto como etiqueta principal. Mantenemos
    // el trimestre de aplicación aquí para no romper snapshots persistidos.
    date: CBAM_CERTIFICATE_PRICE_DATE,
    publicationDate: CBAM_CERTIFICATE_PRICE_PUBLICATION_DATE,
    applicationQuarter: CBAM_CERTIFICATE_PRICE_APPLICATION_QUARTER,
    regulatoryRef: CBAM_CERTIFICATE_PRICE_REGULATORY_REF,
    // Texto cliente-friendly por defecto. El PDF actual ya no renderiza
    // este campo (usa priceNoteText basado en lookupSource para tener
    // mensajes condicionales en `lib/cbamReportGenerator.jsx`), pero se
    // mantiene en el contrato del objeto para retrocompatibilidad de
    // snapshots legacy y de cualquier consumer indirecto. Sin jerga
    // técnica — no mencionar nombres internos de tabla/función.
    note: 'Precio del certificado CBAM aplicable al trimestre de referencia.',
  }
}
...
export function getRegulatoryParamsForYear(year) {
  ...
  return {
    ...
    certificatePricePublicationDate: price.publicationDate,
    certificatePriceApplicationQuarter: price.applicationQuarter,
    certificatePriceRegulatoryRef: price.regulatoryRef,
