...
/**
 * Resuelve el precio del certificado CBAM con lectura híbrida:
 *  1. Fuente primaria: cbam_ets_prices.is_current vía getCurrentETSPrice().
 *  2. Respaldo: constante CBAM_CERTIFICATE_PRICE_FALLBACK
 *     (lib/cbamRegulatoryParams.js) cuando la BD no está disponible
 *     (build time, tests, env vars faltantes, fallo de red).
 *
 * Devuelve { price, lookupSource, date } donde lookupSource indica de
 * dónde se leyó el valor en runtime ('database' o 'fallback'). NO confundir
 * con la referencia regulatoria del precio (Reg. Ejec. 2025/2548), que
 * vive en regParams.certificatePriceRegulatoryRef.
 *
 * Nunca lanza: peor caso devuelve el respaldo. Origen registrado para
 * auditoría del informe.
 */
async function resolveCbamPrice(regParams) {
  const fallback = {
    price: regParams.certificatePrice,
    lookupSource: 'fallback',
    date: regParams.certificatePriceDate,
    applicationQuarter: regParams.certificatePriceApplicationQuarter,
  }
  try {
    const dbPrice = await getCurrentETSPrice()
    if (dbPrice && typeof dbPrice.price === 'number' && dbPrice.price > 0) {
      return {
        price: dbPrice.price,
        lookupSource: 'database',
        date: dbPrice.date || regParams.certificatePriceDate,
        applicationQuarter:
          dbPrice.applicationQuarter || regParams.certificatePriceApplicationQuarter,
      }
    }
    return fallback
  } catch (err) {
    safeLogger.warn(
      '[cbamCalculator] No se pudo leer cbam_ets_prices.is_current, usando fallback:',
      err?.message,
    )
    return fallback
  }
...
// La matemática por línea vive en su propio fichero sin imports para que
// sea testeable con `node --test` sin toolchain de Next. Re-export aquí
// para que los consumidores actuales no rompan.
export { calculateLineMath } from '@/lib/cbamLineMath'
import { calculateLineMath } from '@/lib/cbamLineMath'
...
export async function calculateProducts({ products, year = 2026 }) {
  ...
  // Precio con lectura híbrida BD → fallback. Nunca lanza: peor caso devuelve
  // el respaldo regulatorio constante. El origen viaja al snapshot para que
  // el informe sea auditable.
  const priceInfo = await resolveCbamPrice(regParams)
  const cbamPrice = priceInfo.price
  const cbamPriceLookupSource = priceInfo.lookupSource
  const cbamPriceDate = priceInfo.date
  const cbamPriceApplicationQuarter = priceInfo.applicationQuarter

  const lines = []
  let totalCost = 0
  let totalTonnes = 0
  let totalEmissions = 0
  let totalCertificatesPhysical = 0
  let totalCertificatesToSurrender = 0
  ...
  for (const product of products) {
    ...
    if (product.hasRealEmissions && product.emissionFactorReal) {
      ...
      if (officialDV) {
        ...
        safeLogger.warn(`[cbamCalculator] No official DV for CN ${cnCode}, country ${countryCode}`)
        emissionFactor = 0
        emissionFactorRaw = 0
    ...
    const incorporatedEmissions = lineMath.incorporatedEmissions
    const effectiveBenchmark = lineMath.effectiveBenchmark
    const freeAllocationImplicit = lineMath.freeAllocationImplicit
    const certificatesPhysical = lineMath.certificatesPhysical
    const certificatesToSurrender = lineMath.certificatesToSurrender
    const lineCost = lineMath.lineCost

    // 5. Escenario dual: SIEMPRE calcular coste con defaults para comparar.
    // Reusa la misma matemática (ceil por línea sobre el raw, no decimal).
    ...
    const defaultMath = calculateLineMath({
      tonnes: annualTonnes,
      emissionFactor: defaultFE,
      benchmark: defaultBenchmark,
      cbamFactor,
      fci,
      price: cbamPrice,
    })
    const costWithDefault = defaultMath.lineCost
    ...
    lines.push({
      ...
      // Aliases retrocompatibles. Apuntan a la magnitud física en tCO₂e (no
      // al entero), para que la calculadora pública y cbamDiagnosticBuilder
      // sigan presentando decimales como antes.
      certificates: certificatesPhysical,
      agie: freeAllocationImplicit,
      totalCost: lineCost,
      markupApplied,
      costWithReal,
      costWithDefault,
    ...
    if (hasOfficialData) {
      ...
      totalEmissions += lineEmissions
      // suma de ceils por línea — NUNCA ceil(suma). Cada línea es una
      // entrega independiente (en el futuro con su precio trimestral propio).
      totalCertificatesPhysical += certificatesPhysical
      totalCertificatesToSurrender += certificatesToSurrender
    }
  }
  ...
  return {
    ...
    totals: {
      ...
      totalCertificatesPhysical: Math.round(totalCertificatesPhysical * 100) / 100,
      totalCertificatesToSurrender, // entero ya acumulado, no redondear
      exceedsDeMinisMis: totalTonnes >= 50,
      co2PriceUsed: cbamPrice,
      co2PriceLookupSource: cbamPriceLookupSource,
      co2PriceDate: cbamPriceDate,
      co2PriceApplicationQuarter: cbamPriceApplicationQuarter,
    },
    regParams: {
      year,
      cbamFactor,
      ...
      certificatePriceLookupSource: cbamPriceLookupSource,
      certificatePriceDate: cbamPriceDate,
      certificatePriceApplicationQuarter: cbamPriceApplicationQuarter,
    },
  }
}
...
export async function calculateAdvisoryRequest(requestId, client) {
  ...
  // certificatesPhysical y freeAllocationImplicit se persisten para que el
  // snapshot del informe lea, en vez de recalcular (fuente única de verdad).
  for (const line of lines) {
    await updateAdvisoryProduct(line.id, {
      ...
      certificatesPhysical: line.certificatesPhysical,
      certificatesToSurrender: line.certificatesToSurrender,
      freeAllocationImplicit: line.freeAllocationImplicit,
    }, client)
  }
