...
export async function calculateProducts({ products, year = 2026 }) {
  ...
  for (const product of products) {
    ...
    //
    // Fuente única de verdad: calculamos aquí TODAS las magnitudes derivadas
    // (incorporated, effective benchmark, AGIE y certificados físicos) y las
    // persistimos. El snapshot del informe ya no recalcula — lee del producto.
    const emissionsSubject = Math.max(0, emissionFactor - benchmark)
    const lineEmissions = annualTonnes * emissionsSubject

    // Emisiones incorporadas (Tn × FE), antes de descontar AGIE.
    const incorporatedEmissions = annualTonnes * emissionFactor

    // Benchmark efectivo (F_CBAM × FCI × BM) y AGIE (free allocation implícita).
    const effectiveBenchmark = cbamFactor * fci * benchmark
    const freeAllocationImplicit = annualTonnes * effectiveBenchmark

    // Certificados físicos: magnitud en tCO₂e (decimal). El redondeo a
    // certificados enteros (Math.ceil por Art. 20 Reg. 2023/956) llega en
    // Bloque 3. Hoy: coste = certificatesPhysical × precio (sin cambio).
    const certificatesPhysical = Math.max(0, incorporatedEmissions - freeAllocationImplicit)
    const lineCost = certificatesPhysical * cbamPrice
    ...
    const certificatesPhysicalDefault = annualTonnes * certPerTonneDefault
    const costWithDefault = certificatesPhysicalDefault * cbamPrice
    ...
    lines.push({
      ...
      // Alias retrocompatible — consumidores existentes (snapshot legacy,
      // testCBAME2E.js) leen `certificates`. Mantenemos hasta cerrar Bloque 3.
