...
export async function getGlossaryTermBySlug(slug) {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      updated_at, published_at, seo_ready,
      category:glossary_categories(slug, name_es, name_en),
...
/**
 * Slugs publicados con `seo_ready` para construir el sitemap.
 * Incluye todos los publicados (no solo seo_ready) para que Google
 * descubra las URLs; las páginas no-ready emiten noindex,follow.
 * @returns {Promise<Array<{slug: string, updated_at: string, seo_ready: boolean}>>}
 */
export async function listGlossarySlugsForSitemap() {
  const { data, error } = await supabase
    .from('glossary_terms')
    .select('slug, updated_at, seo_ready')
    .eq('published', true)

  if (error) throw new Error(`listGlossarySlugsForSitemap: ${error.message}`)
  return data || []
}
