...
function mapRequest(row) {
  ...
  return {
    ...
    totalCertificatesPhysical: row.total_certificates_physical != null ? parseFloat(row.total_certificates_physical) : null,
    totalCertificatesToSurrender: row.total_certificates_to_surrender != null ? parseInt(row.total_certificates_to_surrender, 10) : null,
    ...
    co2PriceApplicationQuarter: row.co2_price_application_quarter || null,
...
function toSnake(obj) {
  const map = {
    ...
    co2PriceApplicationQuarter: 'co2_price_application_quarter',
...
export async function listAllAdvisoryRequests({ status = null, search = null } = {}) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error listando:', error.message)
...
export async function getAdvisoryRequestAsAdmin(requestId) {
  ...
  if (reqResult.error) {
    safeLogger.error('[Advisory Admin] Error obteniendo solicitud:', reqResult.error.message)
...
export async function updateAdvisoryRequestAsAdmin(requestId, data) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error actualizando solicitud:', error.message)
...
export async function addAdvisoryProductAsAdmin(requestId, data) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error añadiendo producto:', error.message)
...
export async function updateAdvisoryProductAsAdmin(productId, data) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error actualizando producto:', error.message)
...
export async function deleteAdvisoryProductAsAdmin(productId) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error eliminando producto:', error.message)
...
export async function generateReportRef() {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error contando refs:', error.message)
...
export async function createReportVersion({
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error creando versión informe:', error.message)
...
export async function getCurrentReportForRequest(requestId) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error obteniendo informe:', error.message)
...
export async function listReportsForRequest(requestId) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error listando informes:', error.message)
...
export async function uploadReportPdf({ requestId, filename, pdfBuffer }) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error subiendo PDF:', error.message)
...
export async function getReportSignedUrl(pdfPath, expiresInSeconds = 3600) {
  ...
  if (error) {
    safeLogger.error('[Advisory Admin] Error generando signed URL:', error.message)
...
export async function logReportDownload({
  ...
  if (error) {
    ...
    safeLogger.error('[Advisory Admin] Error registrando descarga:', error.message)
  }
}
