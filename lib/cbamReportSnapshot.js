...
export function buildReportSnapshot({ advisory, reportRef, generatedBy }) {
  ...
  // Detector de informes pre-Bloque 3: si ninguna línea trae el campo
  // certificatesToSurrender desde BD, el PDF mostrará solo la columna de
  // tCO₂e con la nota "[Informe v3.0 pre-redondeo]".
  let anyLineHasSurrender = false
  ...
  const productSnapshots = products.map((p, idx) => {
    ...
    const persistedSurrender = p.certificatesToSurrender != null ? parseInt(p.certificatesToSurrender, 10) : null
    ...
    // certificatesToSurrender: leer del producto. Si NULL (línea legacy),
    // el snapshot expone null y el PDF cae a modo "solo tCO₂e".
    const lineSurrender = persistedSurrender != null ? persistedSurrender : null
    if (lineSurrender != null) anyLineHasSurrender = true
    ...
    if (lineSurrender != null) totalCertificatesToSurrender += lineSurrender
    ...
    return {
      ...
      certificatesToSurrender: lineSurrender, // null en líneas legacy pre-Bloque 3
      // Alias retrocompatible: el PDF v3 actual lee `certificatesAfterAdjustment`.
      // Se mantiene durante el periodo de gracia (ver ROADMAP_v3_features.md).
  ...
  const totals = {
    ...
    // Magnitud física: suma de certificadosPhysical decimales por línea.
    // (Conserva el nombre `totalCertificates` por retrocompat — apuntaba al
    // mismo valor antes de Bloque 3.)
    totalCertificates: round2(totalCertificates),
    totalCertificatesPhysical: round2(totalCertificates),
    // Suma de los ceils por línea — NUNCA ceil del total. null en informes
    // legacy completos (ninguna línea trae el campo desde BD).
    totalCertificatesToSurrender: anyLineHasSurrender ? totalCertificatesToSurrender : null,
    ...
    // Flag de informe pre-Bloque 3: si false, el PDF muestra solo tCO₂e
    // con nota "[Informe v3.0 pre-redondeo]".
    hasCertificatesToSurrender: anyLineHasSurrender,
  }
  ...
  const snapshot = {
    meta: {
      ...
      regulatoryParams: {
        ...
        // Trimestre de aplicación del precio (NO inferido desde la fecha de
        // publicación): persistido al calcular, con respaldo en la constante
        // regulatoria si el snapshot es anterior a la columna o el cálculo
        // se hizo sin la fuente DB disponible.
        certificatePriceApplicationQuarter:
          advisory.co2PriceApplicationQuarter ||
          regParams.certificatePriceApplicationQuarter,
        // Fecha de publicación oficial (ISO YYYY-MM-DD) — solo informativa,
        // se muestra como secundaria en la tabla de metodología.
        certificatePricePublicationDate: regParams.certificatePricePublicationDate,
