...
import {
  parseDutyConditions,
  interpretConditions,
  interpretMeasureConditions,
  getMeasureCategory,
  determineSeverity
} from '@/lib/measureInterpreter'
...
export async function calculateTariff({
  ...
  mode = 'full', // 'full' = calculadora normal, 'lite' = bulk (sin alertas, condiciones, footnotes)
  _originGroupsCache = null // Cache externo de grupos geográficos (para bulk)
}) {
  ...
  const measures = await getApplicableMeasures(paddedHsCode, normalizedCountryCode, refDate, isLite, _originGroupsCache)
  ...
  // Obtener descripción jerárquica (solo en modo full)
  const fullDescription = isLite
    ? await getSimpleDescription(paddedHsCode)
    : await getHierarchicalDescription(paddedHsCode)
  ...
  // Modo lite: respuesta ligera sin alertas, condiciones, footnotes
  if (isLite) {
    return {
      success: true,
      data: {
        ...
        hasAntidumping: !!measures.antidumping,
        antidumpingRate: measures.antidumping?.rate || 0,
        vat: {
          rate: vatRate,
          type: vatType,
          amount: vatAmount
  ...
  // Obtener alertas (controles, restricciones, etc.) — solo modo full
  const alerts = await getApplicableAlerts(paddedHsCode, normalizedCountryCode, refDate)

  // Obtener requisitos documentales interpretados — solo modo full
  const documentRequirements = await getDocumentRequirements(paddedHsCode, normalizedCountryCode, refDate)
 ...
 */
export async function resolveOriginGroups(countryCode, refDate) {
  ...
  if (error || !memberships) {
    safeLogger.error('Error resolviendo grupos geográficos:', error)
 ...
 */
async function getApplicableMeasures(goodsCode, originCode, refDate, lite = false, originGroupsCache = null) {
  const result = {
  ...
  // Resolver todos los origin codes aplicables al país (usar cache si disponible)
  const originGroups = originGroupsCache || await resolveOriginGroups(originCode, refDate)
  ...
  for (let len = 10; len >= 4; len -= 2) {
    ...
    for (const m of measures) {
      ...
      if ((typeCode === 103 || typeCode === 106)) {
        if (mOrigin === '1011' || mOrigin === '1008') {
          if (!result.baseDuty && !isExcluded(typeCode, mOrigin)) {
            let baseCondDetails = null

            // En modo lite, saltar búsqueda de condiciones/certificados
            if (!lite) {
              const { data: baseConds } = await supabase
              ...
              if (baseConds && baseConds.length > 0) {
                const certCodes = [...new Set(baseConds.map(c => c.certificate_code))]
                ...
                baseCondDetails = baseConds.map(c => ({
        ...
        if (!isExcluded(typeCode, mOrigin)) {
          if (!result.preference || (isDirect && result.preference._fromGroup)) {
            ...
            if ([143, 146, 147].includes(typeCode) && m.order_no) {
              ...
              let prefCondDetails = null

              // En modo lite, saltar búsqueda de condiciones/certificados
              if (!lite) {
                const { data: prefConds } = await supabase
                  .from('measure_conditions')
                  .select('certificate_code, condition_code, action_code')
                  .eq('goods_code', searchCode)
                  .eq('measure_type_code', typeCode)
                ...
                if (prefConds && prefConds.length > 0) {
                  const certCodes = [...new Set(prefConds.map(c => c.certificate_code))]
                  const { data: certDescs } = await supabase
                    .from('certificate_types')
                    .select('certificate_code, description')
                    .in('certificate_code', certCodes)
                  ...
                  const certMap = new Map()
                  if (certDescs) certDescs.forEach(cd => certMap.set(cd.certificate_code, cd.description))
                  prefCondDetails = prefConds.map(c => ({
                    certificate: c.certificate_code,
                    description: certMap.get(c.certificate_code) || translateCertificate(c.certificate_code).text,
                    conditionCode: c.condition_code,
                    actionCode: c.action_code
      ...
      // Additional code — enriquecer con descripción si existe (solo modo full)
      if (!lite && m.add_code && !result._addCodeDesc) {
...
function determineFinalDuty(measures, countryData, goodsCode) {
  ...
  let conditions = measures.baseDuty?.conditions || null
  ...
  if (measures.suspension &&
    ...
    conditions = measures.suspension.conditions || conditions
  }
  ...
  if (measures.preference) {
    ...
    conditions = measures.preference.conditions || conditions
  }
...
/**
 * Obtiene solo la descripción del nivel más específico (para modo lite/bulk).
 * Una sola query en vez de 5 niveles jerárquicos.
 */
async function getSimpleDescription(paddedHsCode) {
  const { data: desc } = await supabase
    .from('descriptions')
    .select('description')
    .eq('goods_code', paddedHsCode)
    .maybeSingle()

  return desc?.description || `Código HS: ${paddedHsCode}`
}
...
async function getApplicableAlerts(goodsCode, originCode, refDate) {
  ...
  try {
    ...
    // Buscar con cascada de código (10→8→6→4) para capturar notas a nivel padre
    let footnotes = null
    for (let fnLen = 10; fnLen >= 4; fnLen -= 2) {
      const fnCode = goodsCode.substring(0, fnLen).padEnd(10, '0')
      const { data: fnData } = await supabase
        .from('measure_footnotes')
        .select('footnote_code, measure_type_code, measure_type_name')
        .eq('goods_code', fnCode)
        .limit(20)
    ...
    safeLogger.error('Error obteniendo alertas:', error)
  }
...
/**
 * Obtiene los requisitos documentales interpretados para una partida y país.
 * Consulta ambas fuentes: duty_expression (Fuente 1) y measure_conditions (Fuente 2).
 * Enriquece con certificate_types y footnotes.
 */
async function getDocumentRequirements(goodsCode, originCode, refDate) {
  try {
    // Resolver grupos del país
    const originGroups = await resolveOriginGroups(originCode, refDate)

    // 1. Obtener todas las medidas con condiciones (Fuente 1: duty_expression con Cond:)
    const { data: measures } = await supabase
      .from('taric_measures')
      .select('measure_type_code, measure_type_name, duty_expression, origin_code, origin_name, legal_base')
      .eq('goods_code', goodsCode)
      .like('duty_expression', 'Cond:%')
      .lte('start_date', refDate)
      .or(`end_date.is.null,end_date.gte.${refDate}`)

    // 2. Obtener condiciones estructuradas (Fuente 2: measure_conditions)
    const { data: structuredConds } = await supabase
      .from('measure_conditions')
      .select('measure_type_code, condition_code, condition_sequence, certificate_code, condition_amount, measurement_unit, action_code, origin_code')
      .eq('goods_code', goodsCode)
    ...
    const isExcluded = (typeCode, mOrigin) => {
      ...
      return exclusions.some(e =>
        e.measure_type_code === typeCode &&
        e.origin_code === String(mOrigin) &&
        e.excluded_country_code === originCode
      )
    }

    // 4. Collect all certificate codes we'll need descriptions for
    const allCertCodes = new Set()

    // From Fuente 1
    const parsedByMeasure = new Map()
    if (measures) {
      for (const m of measures) {
        const parsed = parseDutyConditions(m.duty_expression)
        if (parsed) {
          parsedByMeasure.set(m, parsed)
          for (const cert of parsed.certificates) {
            allCertCodes.add(cert.certificate)
          }
        }
      }
    }
    ...
    // From Fuente 2
    if (structuredConds) {
      for (const row of structuredConds) {
        if (row.certificate_code) allCertCodes.add(row.certificate_code)
      }
    }

    // 5. Batch fetch certificate descriptions (ES)
    const certDescriptions = new Map()
    if (allCertCodes.size > 0) {
      const { data: certDescs } = await supabase
        .from('certificate_types')
        .select('certificate_code, description')
        .in('certificate_code', [...allCertCodes])
        .eq('language', 'ES')

      if (certDescs) {
        certDescs.forEach(cd => certDescriptions.set(cd.certificate_code, cd.description))
      }
    }

    // 6. Fetch footnotes for measures with conditions
    const measureTypeCodes = new Set()
    if (measures) measures.forEach(m => measureTypeCodes.add(m.measure_type_code))
    if (structuredConds) structuredConds.forEach(c => measureTypeCodes.add(c.measure_type_code))

    const footnoteMap = new Map() // measureTypeCode → footnote text[]
    if (measureTypeCodes.size > 0) {
      const { data: footnotes } = await supabase
        .from('measure_footnotes')
        .select('measure_type_code, footnote_code')
        .eq('goods_code', goodsCode)
        .in('measure_type_code', [...measureTypeCodes])
      ...
      if (footnotes && footnotes.length > 0) {
        ...
        // Group by measure_type_code, deduplicate
        const seenByType = new Map()
        for (const fn of footnotes) {
          const desc = fnDescMap.get(fn.footnote_code)
          if (!desc) continue
          if (!seenByType.has(fn.measure_type_code)) seenByType.set(fn.measure_type_code, new Set())
          if (seenByType.get(fn.measure_type_code).has(fn.footnote_code)) continue
          seenByType.get(fn.measure_type_code).add(fn.footnote_code)

          if (!footnoteMap.has(fn.measure_type_code)) footnoteMap.set(fn.measure_type_code, [])
          footnoteMap.get(fn.measure_type_code).push(desc)
        }
      }
    }

    // 7. Build document requirements
    const requirements = []
    const processedTypes = new Set() // Avoid duplicating same measure_type

    // Process Fuente 1 (duty_expression)
    if (measures) {
      for (const m of measures) {
        const mOrigin = String(m.origin_code)

        // Filter by origin: measure must apply to the user's country/group
        if (!originGroups.includes(mOrigin)) continue
        // Filter by exclusions
        if (isExcluded(m.measure_type_code, mOrigin)) continue

        const parsed = parsedByMeasure.get(m)
        if (!parsed) continue

        const conditionGroups = interpretConditions(parsed, certDescriptions)
        if (conditionGroups.length === 0) continue

        const measureInfo = translateMeasureType(String(m.measure_type_code))
        const category = getMeasureCategory(m.measure_type_code)
        const severity = determineSeverity(m.measure_type_code, conditionGroups)
        const notes = footnoteMap.get(m.measure_type_code) || []

        requirements.push({
          measureType: m.measure_type_code,
          measureName: measureInfo.text,
          measureIcon: measureInfo.icon,
          category,
          severity,
          conditionGroups,
          originCode: mOrigin,
        ...
        processedTypes.add(m.measure_type_code)
      }
    }

    // Process Fuente 2 (measure_conditions) — only for types not already processed
    if (structuredConds) {
      // Group by measure_type_code + origin_code
      const groupedConds = new Map()
      for (const row of structuredConds) {
        if (processedTypes.has(row.measure_type_code)) continue
        const mOrigin = String(row.origin_code)
        if (!originGroups.includes(mOrigin)) continue
        if (isExcluded(row.measure_type_code, mOrigin)) continue

        const key = `${row.measure_type_code}|${mOrigin}`
        if (!groupedConds.has(key)) groupedConds.set(key, { rows: [], typeCode: row.measure_type_code, origin: mOrigin })
        groupedConds.get(key).rows.push(row)
      }

      for (const [, group] of groupedConds) {
        const conditionGroups = interpretMeasureConditions(group.rows, certDescriptions)
        if (conditionGroups.length === 0) continue

        const measureInfo = translateMeasureType(String(group.typeCode))
        const category = getMeasureCategory(group.typeCode)
        const severity = determineSeverity(group.typeCode, conditionGroups)
        const notes = footnoteMap.get(group.typeCode) || []

        requirements.push({
          measureType: group.typeCode,
          measureName: measureInfo.text,
          measureIcon: measureInfo.icon,
          category,
          severity,
          conditionGroups,
          originCode: group.origin,
          originName: null,
          legalBase: null,
          explanatoryNotes: notes,
          source: 'measure_conditions'
        })
      }
    }

    // Sort: restriccion first, then paraaduanero, then cbam, then derechos, then info
    const categoryOrder = { restriccion: 0, paraaduanero: 1, cbam: 2, derechos: 3, info: 4, otro: 5 }
    requirements.sort((a, b) => (categoryOrder[a.category] ?? 5) - (categoryOrder[b.category] ?? 5))

    return requirements
  } catch (error) {
    safeLogger.error('Error obteniendo requisitos documentales:', error)
    return []
  }
}
