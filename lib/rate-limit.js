...
/**
 * Rate limiter para OCR de FACTURAS
 * MUY restrictivo por IP: 5 por hora (la API es cara y procesa archivos)
 * Además hay límite diario por usuario en checkDailyInvoiceOCRLimit
 */
export const invoiceOCRLimiter = redis ? new Ratelimit({
  redis,
  limiter: Ratelimit.slidingWindow(5, '1 h'),
  analytics: true,
  prefix: 'lexaduana:invoice-ocr',
}) : null
...
export async function checkRateLimit(request, limiter, identifier = null) {
  ...
  if (!limiter || !redis) {
    safeLogger.warn('Rate limiting no configurado - permitiendo petición')
    return {
      success: true,
      limit: 0,
      remaining: 0,
  ...
  try {
    ...
    safeLogger.error('Rate limit check failed:', error)
    return {
      success: true,
      limit: 0,
      remaining: 0,
...
export async function checkDailyAILimit(userId) {
  ...
  try {
    ...
    safeLogger.error('Daily AI limit check failed:', error)
    // Fail-open
    return { allowed: true, used: 0, limit: DAILY_LIMIT, remaining: DAILY_LIMIT }
  }
}
...
/**
 * Verifica y actualiza el límite diario de OCR de facturas por usuario.
 * Por defecto: 2/día (free tier). Preparado para tiers futuros vía argumento.
 *
 * @param {string} userId - auth.uid()
 * @param {number} dailyLimit - Límite diario (default 2)
 * @returns {Promise<{allowed: boolean, used: number, limit: number, remaining: number, resetsIn: number}>}
 */
export async function checkDailyInvoiceOCRLimit(userId, dailyLimit = 2) {
  if (!redis) {
    return { allowed: true, used: 0, limit: dailyLimit, remaining: dailyLimit, resetsIn: 86400 }
  }

  const key = `lexaduana:invoice-ocr:daily:${userId}`
  ...
  try {
    ...
    safeLogger.error('Daily invoice OCR limit check failed:', error)
    return { allowed: true, used: 0, limit: dailyLimit, remaining: dailyLimit, resetsIn: 86400 }
  }
}

/**
 * Obtener uso actual de OCR de facturas (sin incrementar)
 */
export async function getUserInvoiceOCRUsage(userId, dailyLimit = 2) {
  if (!redis) {
    return { used: 0, limit: dailyLimit, remaining: dailyLimit, resetsIn: 86400 }
  }

  const key = `lexaduana:invoice-ocr:daily:${userId}`

  try {
    const [usedRaw, ttl] = await Promise.all([redis.get(key), redis.ttl(key)])
    ...
    safeLogger.error('Get invoice OCR usage failed:', error)
    return { used: 0, limit: dailyLimit, remaining: dailyLimit, resetsIn: 86400 }
  }
}
...
export async function getUserAIUsage(userId) {
  ...
  try {
    ...
    safeLogger.error('Get AI usage failed:', error)
    return { used: 0, limit: DAILY_LIMIT, remaining: DAILY_LIMIT, resetsIn: 86400 }
  }
}
