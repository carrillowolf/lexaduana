...
function getResend() {
  ...
  if (!key) {
    safeLogger.warn('[CBAM Emails] RESEND_API_KEY no configurado — emails deshabilitados')
    return null
  }
...
async function safeSend({ to, subject, html, replyTo = REPLY_TO, tag }) {
  ...
  try {
    ...
    if (result.error) {
      safeLogger.error('[CBAM Emails] Error envío:', result.error)
      return { success: false, error: result.error.message }
    }
    return { success: true, id: result.data?.id }
  } catch (err) {
    safeLogger.error('[CBAM Emails] Excepción envío:', err.message)
