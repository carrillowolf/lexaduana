...
export async function getCurrentETSPrice() {
  try {
    const { data, error } = await supabase
      ...
      .select('price, price_date, application_quarter, source, created_at')
      .eq('is_current', true)
      .single()

    if (error || !data) {
      safeLogger.error('[CBAM Service] No se encontró precio ETS actual en DB. Actualiza cbam_ets_prices.')
      return null
    }
    ...
    return {
      ...
      applicationQuarter: data.application_quarter,
      source: data.source
    }
  } catch (err) {
    safeLogger.error('[CBAM Service] Error obteniendo precio ETS:', err.message)
    return null
  }
}
...
export async function getETSPriceHistory(limit = 30) {
  try {
    ...
    safeLogger.error('[CBAM Service] Error obteniendo historial de precios:', err.message)
    return []
  }
}
...
export async function getSectors() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback a sectores hardcoded:', err?.message)
...
export async function getCBAMCodes(includeDownstream = false) {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback a códigos CN hardcoded:', err?.message)
    return CBAM_CODES
  }
}
...
export async function getExcludedCountries() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback a países excluidos hardcoded:', err?.message)
...
export async function getEmissionFactors() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback a factores de emisión hardcoded:', err?.message)
...
export async function getBenchmarksBySector(sectorId) {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback benchmarks:', err?.message)
...
export async function getTimeline() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback timeline:', err?.message)
...
export async function getCertificates() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback certificados:', err?.message)
...
export async function getDefaultValueMarkupSchedule() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback markup schedule:', err?.message)
...
export async function getDownstreamExtension() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback downstream extension:', err?.message)
...
export async function getThreshold() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Fallback threshold:', err?.message)
...
export async function getRegulations() {
  try {
    ...
    safeLogger.warn('[CBAM Service] Error obteniendo reglamentos:', err?.message)
    return []
  }
}
...
export async function updateETSPrice(price, date, source = 'manual') {
  try {
    ...
    safeLogger.error('[CBAM Service] Error actualizando precio ETS:', err.message)
    return false
  }
}
...
export async function getOfficialBenchmark(cnCode, column = 'B', productionRoute = null) {
  ...
  try {
    ...
    safeLogger.error('[CBAM Service] Error getOfficialBenchmark:', err?.message)
    return null
  }
}
...
export async function getOfficialDefaultValue(cnCode, countryCode, year = 2026, productionRoute = null) {
  ...
  // Helper: cascada jerárquica 8 → 6 → 4 dígitos para un país dado
  async function findByCountry(cc) {
    ...
    // Prefix fallback: 6 digits, then 4 digits
    for (const len of [6, 4]) {
  ...
  try {
    // 1. Cascada completa con el país solicitado
    let data = await findByCountry(ccNorm)

    // 2. Fallback a XX (Other Countries) con la misma cascada
    if (!data && ccNorm !== 'XX') {
      data = await findByCountry('XX')
    }

    if (!data?.length) {
      safeLogger.warn(`[CBAM DV] No official default value found for CN ${cnNorm}, country ${ccNorm} (also tried XX fallback)`)
      return null
    }
    ...
    safeLogger.error('[CBAM Service] Error getOfficialDefaultValue:', err?.message)
    return null
  }
}
