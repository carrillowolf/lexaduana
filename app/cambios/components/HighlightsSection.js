...
function generateInsights(chapters, t) {
  ...
  if (topRemoved.length > 0) {
    ...
    insights.push({ icon: '🗑️', text: `${totalRemoved} ${t('insights.removed')} ${topRemoved.slice(0, 3).map(ch => ch.chapter).join(', ')}`, type: 'warning' })
  }
  ...
  if (topAdded.length > 0) {
    ...
    insights.push({ icon: '🆕', text: `${totalAdded} ${t('insights.added')} ${topAdded.length} ${t('insights.addedChapters')}`, type: 'success' })
  }

  const critChapters = [...chapters].filter(ch => ch.critical > 0).sort((a, b) => b.critical - a.critical)
  if (critChapters.length > 0) {
    insights.push({ icon: '⚠️', text: `${t('insights.critical')} ${critChapters.map(ch => `${ch.chapter} (${ch.name})`).slice(0, 4).join(', ')}`, type: 'danger' })
  }
  ...
  if (noiseChapters.length > 0) {
    ...
    insights.push({ icon: '📝', text: `${noiseChapters.length} ${t('insights.footnotes')} (${totalNoise.toLocaleString('es-ES')} ${t('insights.footnotesRecords')})`, type: 'muted' })
  }
...
export default function HighlightsSection({ chapters, t }) {
  const insights = generateInsights(chapters, t)

  if (insights.length === 0) return null

  return (
    <div>
      <h3 className="text-base font-bold text-gray-800 mb-3 px-1">{t('insights.title')}</h3>
